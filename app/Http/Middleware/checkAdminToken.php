...
class checkAdminToken
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        try {
            ...
            $user = JWTAuth::parseToken()->authenticate(); //check token state
                //throw an exception

        } catch (Exception $e) {
            if ($e instanceof \Tymon\JWTAuth\Exceptions\TokenInvalidException){
                //return response() -> json(['success' => false, 'msg' => 'INVALID _TOKEN']);
                return $this->returnErrorMessage('INVALID _TOKEN');
            }else if ($e instanceof \Tymon\JWTAuth\Exceptions\TokenExpiredException){
                //return response() -> json(['success' =>false, 'msg'=>'EXPIRED_TOKEN']);
                return $this->returnErrorMessage('EXPIRED_TOKEN');
            } else{
        ...
        if(!$user)
            return this->returnErrorMessage('Unauthenticated!');
        return $next($request);
    }
}
